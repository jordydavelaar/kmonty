...
#define LOOP_ijk    for(int i = 0; i < NDIM; i++) \
...
void scatter_super_photon(struct of_photon *ph, struct of_photon *php,
			  ...
			  double Gcov[NDIM][NDIM], int ACCZONE);
...
double gdet_func(double gcov[][NDIM], double X[NDIM]);  /* calculated numerically */
...
double alpha_inv_abs_th(double nu, double Thetae, double Ne, double B,double theta);
...
double alpha_inv_abs_nth(double nu, double Thetae, double Ne, double B,double theta);
...
void sample_electron_distr_p(double k[NDIM], double p[NDIM], double theta, int ACCZONE);
void sample_beta_distr(double theta, double *gamma_e, double *beta_e, int ACCZONE);
