...
double interp_scalar(double **var, int i, int j, double coeff[4]) {
  ...
  interp = var[i][j] * coeff[0] + var[i][j + 1] * coeff[1] +
...
#pragma omp parallel private(i, j, Thetae, K2, Ne, B, fac_th, fac_nth, l,      \
                             lstart, lend, myid, nthreads, Ucon, Bcon)
  {
    ...
    for (i = 0; i < N1; i++)
      for (j = 0; j < N2; j++) {
        get_fluid_zone(i, j, &Ne, &Thetae, &B, Ucon, Bcon, &ACCZONE);
...
#elif (KAPPA)
        fac_nth = (EE * EE * EE / (2. * M_PI * ME * CL * CL)) * Ne * B * sfac *
                  geom[i][j].g;
#elif (MIXED)
        K2 = K2_eval(Thetae);
        fac_th = (JCST * Ne * B * Thetae * Thetae / K2) * sfac * geom[i][j].g;
 ...
 */
    ...
    nint[i] *=
        dx[1] * dx[2] * dx[3] * L_unit * L_unit * L_unit * fac_th * 1. / HPL;
#elif (KAPPA)
    double fac_nth = (EE * EE * EE / (2. * M_PI * ME * CL * CL));
    nint[i] *=
        dx[1] * dx[2] * dx[3] * L_unit * L_unit * L_unit * fac_nth * 1. / HPL;
/*#elif (MIXED)
...
static void init_zone(int i, int j, double *nz, double *dnmax) {
  ...
  if (l < 0) {
    ...
    *dnmax = 0.;
    ...
    ninterp *=
        dx[1] * dx[2] * dx[3] * L_unit * L_unit * L_unit * fac_mixed * 1. / HPL;
