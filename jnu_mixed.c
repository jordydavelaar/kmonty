...
double jnu_synch_kappa(double nu, double Ne, double Thetae, double B,
    ...
    double kappa = kappa_synch;
    w = (kappa - 3.) / kappa *
        Thetae; // Thetae; //sqrt(  2./9./kappa *Thetae * Thetae);
...
void init_emiss_tables_th(void) {
    ...
    /*  build table for quick evaluation of the bessel function K2 for
     * emissivity
     */
    for (k = 0; k <= N_ESAMP; k++) {
        T = exp(k * dlT + lT_min);
