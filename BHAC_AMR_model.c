...
int find_cell(double x[4], struct block *block_info, int igrid) {
  ...
  int i =
      (int)((x[1] - block_info[igrid].lb[0]) / block_info[igrid].dxc_block[0]);
  int j =
      (int)((x[2] - block_info[igrid].lb[1]) / block_info[igrid].dxc_block[1]);
  int k =
      (int)((x[3] - block_info[igrid].lb[2]) / block_info[igrid].dxc_block[2]);
...
*/
  ...
  calc_coord(c, nx, ndimini, block_info[igrid].lb, block_info[igrid].dxc_block,
                 Xcent);
  ...
  *dx_local = block_info[igrid].dxc_block[0];
  ...
  double a2 = a * a;
  double r2 =
      (R2 - a2 + sqrt((R2 - a2) * (R2 - a2) + 4. * a2 * Xcent[3] * Xcent[3])) *
      0.5;

  if(sqrt(r2) < 1+sqrt(1-a*a) ||
      sqrt(r2) > 40. || sqrt(r2) < 2) { // || !(rho > 1.0e-5*pow(sqrt(r2),-1.5))){
      *Ne = 0;
      *B = 0;
      *Thetae = 0;
      *dx_local=1e100;
...
*/
  ...
  *B = sqrt(fabs(Bcon[0] * Bcov[0] + Bcon[1] * Bcov[1] + Bcon[2] * Bcov[2] +
           ...
           B_unit +
       smalll;
  double Bsq = fabs(Bcon[0] * Bcov[0] + Bcon[1] * Bcov[1] + Bcon[2] * Bcov[2] +
                    Bcon[3] * Bcov[3]);
#if (DEBUG)
  if (isnan(Bsq) || isnan(*B)) {
    double R2 = Xcent[0] * Xcent[0] +
                Xcent[1] * Xcent[1] +
                Xcent[2] * Xcent[2];
    double a2 = a * a;
    double r2 =
        (R2 - a2 + sqrt((R2 - a2) * (R2 - a2) +
                        4. * a2 * Xcent[2] * Xcent[2])) *
        0.5;
    ...
    fprintf(stderr, "B isnan Bsq %e Bcon %e %e %e %e Ucon %e %e %e %e\n", Bsq,
            Bcon[0], Bcon[1], Bcon[2], Bcon[3], Ucon[0], Ucon[1], Ucon[2],
            Ucon[3]);
...
*/
  ...
  double r2 =
      ...
      0.5;
if(sqrt(r2) < Rh ||
      sqrt(r2) > 75) { // || !(rho > 1.0e-5*pow(sqrt(r2),-1.5))){
      *Ne = 0;
      *B = 0;
      *Thetae = 0;
      *dx_local=1e100;
...
void gcon_func(double *X, double gcon[][NDIM]) {
  ...
  double f = 2. * r2 * r * isig; //(r2*r2+a2*X[3]*X[3]);

  double l[4];

  l[0] = -1.;
  l[1] = (r * X[1] + a * X[2]) * idel; //(r2+a2);
  l[2] = (r * X[2] - a * X[1]) * idel; //(r2+a2);
  l[3] = (X[3]) / (r);
...
void get_connection(double X[4], double lconn[4][4][4]) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // memcpy(&X_u_temp, X, 4 * sizeof(double));
  ...
  for (alpha = 0; alpha < 4; alpha++) {
    for (k = 0; k < 4; k++) {
      ...
      lconn[alpha][0][1] +=
          g_uu[alpha][k] * (0.5 * (dg[1][k][0] + dg[0][k][1] - dg[k][0][1]));
      lconn[alpha][0][2] +=
          g_uu[alpha][k] * (0.5 * (dg[2][k][0] + dg[0][k][2] - dg[k][0][2]));
      lconn[alpha][0][3] +=
          g_uu[alpha][k] * (0.5 * (dg[3][k][0] + dg[0][k][3] - dg[k][0][3]));

      lconn[alpha][1][1] +=
          g_uu[alpha][k] * (0.5 * (2. * dg[1][k][1] - dg[k][1][1]));
      lconn[alpha][1][2] +=
          g_uu[alpha][k] * (0.5 * (dg[2][k][1] + dg[1][k][2] - dg[k][1][2]));
      lconn[alpha][1][3] +=
          g_uu[alpha][k] * (0.5 * (dg[3][k][1] + dg[1][k][3] - dg[k][1][3]));

      lconn[alpha][2][2] +=
          g_uu[alpha][k] * (0.5 * (2. * dg[2][k][2] - dg[k][2][2]));
      lconn[alpha][2][3] +=
          g_uu[alpha][k] * (0.5 * (dg[3][k][2] + dg[2][k][3] - dg[k][2][3]));
...
int stop_criterion(struct of_photon *ph) {
  ...
  double a2 = a * a;
  double r2 =
      (R2 - a2 + sqrt((R2 - a2) * (R2 - a2) + 4. * a2 * ph->X[3] * ph->X[3])) *
      0.5;
  double r = sqrt(r2);
...
#pragma omp critical (MAXTAU)
         ...
         */
        double r = sqrt(ph->X[1]*ph->X[1]+ph->X[2]*ph->X[2]+ph->X[3]*ph->X[3]);
        double theta = (acos(ph->X[3]/r));
        double phi = fmod(atan2(ph->X[2],ph->X[1]),2*M_PI);
        if(phi<0) phi += 2* M_PI;
        if(theta<0 || theta>M_PI) fprintf(stderr,"issue with theta %e\n",theta);
...
#pragma omp atomic
     ...
     /*   if(exp(lE)*ME*CL*CL/HPL < pow(10,19) && exp(lE)*ME*CL*CL/HPL > pow(10,9)) {
                Xi_spec[ii][jj][kk]+=ph->w/(pow(L_unit,3)); // *dix1*dix2*dix3*g);//WEIGHT_MIN/WEIGHT_MIN;
