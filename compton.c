...
double monty_rand() { return (gsl_rng_uniform(r)); }
...
void sample_scattered_photon(double k[4], double p[4], double kp[4]) {
    ...
    */
    ...
    double kmag = sqrt(ke[1] * ke[1] + ke[2] * ke[2] + ke[3] * ke[3]);
    v0x = ke[1] / kmag;
    v0y = ke[2] / kmag;
    v0z = ke[3] / kmag;

    /*being paranoia... It should be 1, but numerical roundoff errors.. */
...
void boost(double v[4], double u[4], double vp[4]) {
    ...
    /*
       if(vp[0]<0) {
       printf("boost vp<0 %e \n",u[0] * v[0] - u[1] * v[1] - u[2] * v[2] - u[3]
       * v[3]); printf("boost udotu %e \n",u[0] * u[0] - u[1] * u[1] - u[2] *
       u[2] - u[3] * u[3]); printf("boost vdotv %e \n",v[0]* v[0] - v[1] * v[1]
       - v[2] * v[2] - v[3] * v[3]); printf("boost u  %e %e %e
       %e\n",u[0],u[1],u[2],u[3]); printf("boost v  %e %e %e
       %e\n",v[0],v[1],v[2],v[3]);
       }
       */
...
void sample_electron_distr_p(double k[4], double p[4], double Thetae,
    ...
    do {
        //    sample_beta_distr(Thetae, &gamma_e, &beta_e, ACCZONE);
        ...
        if (sample_cnt > 10000000) {
            fprintf(stderr,
                    "in sample_electron mu, gamma_e, K, sigma_KN, x1: %g %g "
                    "%g %g %g %g\n",
    ...
    double norm;
    norm = p[0] * p[0] - p[1] * p[1] - p[2] * p[2] - p[3] * p[3];
    p[0] /= sqrt(norm);
    p[1] /= sqrt(norm);
    p[2] /= sqrt(norm);
    p[3] /= sqrt(norm);
    norm = p[0] * p[0] - p[1] * p[1] - p[2] * p[2] - p[3] * p[3];
    // printf("norm %0.15e\n",norm);
    //        if(fabs(p[0]*p[0]-p[1]*p[1]-p[2]*p[2]-p[3]*p[3]-1)>0)
    //                                        printf("%e\n",p[0]*p[0]-p[1]*p[1]-p[2]*p[2]-p[3]*p[3]-1);

    if (gamma_e < 0)
        fprintf(stderr, "gammae error: %e\n", gamma_e);
