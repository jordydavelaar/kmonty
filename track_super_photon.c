...
//    if (Ne > 0.) {
      ...
      if (bound_flag || nu < 0.) {
        ...
        bi = bf;
        if (dtau_abs < 0) {
          fprintf(stderr, "dtau smaller than zero, namely alpha_avg %e dtau "
                          "%e, te %e, ne %e, b %e\n",
                  (alpha_absf + alpha_absi) * 0.5, dtau_abs, Thetae, Ne, B);
        }
      }
      if (ph->w > 1e75)
        fprintf(stderr, "FAULTY photon here, dtau is %e total tau %e", dtau,
      ...
      if (bias * dtau_scatt > x1 && php.w > WEIGHT_MIN) {
        if (isnan(php.w) || isinf(php.w)) {
          fprintf(stderr, "w isnan in track_super_photon: Ne, bias, ph->w, "
                          "php.w  %g, %g, %g, %g\n",
                  Ne, bias, ph->w, php.w);
        }
	...
	push_photon(Xi, Ki, dKi, dl * frac, &E0, 0);
        ...
        gcov_func(ph->X, Gcov);
        get_fluid_params(ph->X, Gcov, &Ne, &Thetae, &B, Ucon,
                         Ucov, Bcon, Bcov, &ACCZONE);

        if (Ne > 0.) {
          scatter_super_photon(ph, &php, Ne, Thetae, B, Ucon, Bcon, Gcov,
                               ACCZONE);
          if (ph->w <
              1.e-100) { /* must have been a problem popping k back onto light
                            cone */
            return;
          }
    ...
    if (nstep > MAXNSTEP) {
      fprintf(stderr, "X1,X2,X3,K1,K2,K3,bias,steps: %g %g %g %g %g %g %g %g %d\n",
              ph->X[1], ph->X[2], ph->X[3], ph->K[1], ph->K[2], ph->K[3], bias,
              dl,nstep);
      break;
    }
  }
