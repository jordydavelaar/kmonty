...
/*
   ...
   kmonty, Monte Calro code based on grmonty (Dolence et al. 2007)

   Code modified by J. Davelaar (c) 2021
   ...
   Input simulation data is assumed to be in postrad format provided by
   BHAC code.
   ...
   Modifications with respect to original code include:
   MPI and OpenMP parallel
   kappa and powerlaw distribution functions
   coupled to BHAC (Porth et al. 2017)
...
#pragma omp atomic
                        ...
                        /* give interim reports on rates */
#if OPENMP
                       if (((int) (N_superph_made_local)) % 100000 == 0
                            && N_superph_made_local > 0) {
                                currtime = time(NULL);
                                fprintf(stderr, "time %g, rate %g ph/s\n",
                                        (double) (currtime - starttime),
