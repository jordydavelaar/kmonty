...
void gcov_func(double X[NDIM], double gcov[NDIM][NDIM]) {
  ...
  gcov[2][2] = pow(X[1], 2) + 1e-10;

  gcov[3][3] = pow(X[1] * sin(X[2]), 2) + 1e-10;
}
...
double stepsize(double X[NDIM], double K[NDIM], double tau) {
  ...
  dlx1 = EPS  / (fabs(K[1]) + SMALL); // *fmin(fmax(exp(-tau),1e-6),1);
  dlxx1 = EPS *X[1]/ (fabs(K[1]) + SMALL); // *fmin(fmax(exp(-tau),1e-6),1);
