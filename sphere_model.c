...
void get_fluid_zone_new(int i, int j, int k, double *Ne, double *Thetae,
                        double *B, double Ucon[NDIM], double Bcon[NDIM],
                        int *ACCZONE) {
    ...
    get_fluid_params(X, gcov, Ne, Thetae, B, &sigma, &beta, Ucon, Ucov, Bcon,
                     Bcov, ACCZONE, &dx_local, &igrid_c);
}
...
int get_fluid_params_new(double X[NDIM], double gcov[NDIM][NDIM], double *Ne,
                         double *Thetae, double *B, double *sigma, double *beta,
                         double Ucon[NDIM], double Ucov[NDIM],
                         double Bcon[NDIM], double Bcov[NDIM], int *ACCZONE,
                         double *dx_local, int *igrid_c) {
    ...
    double model_B0 = CL *
                      sqrt(8 * M_PI * (gam - 1.) * (MP + ME) / MODEL_BETA_0) *
                      sqrt(model_Ne0 * MODEL_THETAE_0) / sqrt(THETAE_UNIT);
...
void report_spectrum(double N_superph_made) {
    ...
    for (i = 0; i < N_EBINS; i++) {
        ...
        //	  fprintf(fp, "%10.5g ", exp(i*dlE +
        //lE0)*ME*CL*CL/HPL/2.417898940e17);
        ////nu in [keV]
        ...
        for (k = 0; k < N_PHIBINS; k++) {
            for (j = 0; j < N_THBINS; j++) {
                ...
                // to fold the sed
#if (FOLDING)
                dOmega = 2. * dOmega_func(j * dx2, (j + 1) * dx2, k * dx3,
                                          (k + 1) * dx3);
#else
                dOmega =
                ...
                /*                               fprintf(fp,"%10.5g %10.5g
                   %10.5g %10.5g %10.5g %10.5g %10.5g ", nuLnu,
                                                       spect[j][k][i].tau_abs /
