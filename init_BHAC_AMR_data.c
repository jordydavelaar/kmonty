...
void read_node(FILE *file_id, int *igrid, int *refine, int ndimini, int level,
  ...
  if (leaf) {
    ...
    for (int ich = 0; ich < pow(2, ndimini); ich++) {
      ...
      read_node(file_id, igrid, refine, ndimini, level + 1, cind_i, cind_j,
                cind_k);
    }
  }
}
...
void calc_coord_bar(double X[4], double *dxc_block, double Xbar[4]) {
  ...
  for (int k = 0; k < 3; k++) {
    for (int j = 0; j < 3; j++) {
      for (int i = 0; i < 3; i++) {
        ...
        xbar +=
            detgamma * coef_3D[i][j][k] * (X[0] + dxc_block[0] / 2. * is[i]);
        ybar +=
            detgamma * coef_3D[i][j][k] * (X[1] + dxc_block[1] / 2. * js[j]);
        zbar +=
            detgamma * coef_3D[i][j][k] * (X[2] + dxc_block[2] / 2. * ks[k]);
  ...
  xbar *=
      (dxc_block[0] / 6.0) * (dxc_block[1] / 6.0) * (dxc_block[2] / 6.0) / norm;
  ybar *=
      (dxc_block[0] / 6.0) * (dxc_block[1] / 6.0) * (dxc_block[2] / 6.0) / norm;
  zbar *=
      (dxc_block[0] / 6.0) * (dxc_block[1] / 6.0) * (dxc_block[2] / 6.0) / norm;
...
void calc_coord(int c, int *nx, int ndimini, double *lb, double *dxc_block,
  ...
  if (isnan(X[0])) {
    fprintf(stderr, "isnan in calccoord %lf %d %lf %lf\n", lb[0], local_ig[0],
            dxc_block[0], ((double)c) / (nx[0] * nx[1]));
    exit(1);
  }
}
...
//	}
  ...
  prim[UU] =
      ...
      (neqpar[0] - 1.); // need internal energy not pressure so extra 1/(gam-1)
  ...
  if (isnan(lor)) {
    ...
    fprintf(stderr, "lor %e vdotv %e lfac %e XI %e Bsq %e BS %e\n", lor, VdotV,
            conserved[LFAC][c], conserved[XI][c], Bsq, BS);
...
fprintf(stderr,"im here!\n");
  for (int i = 0; i < nleafs; i++) {
      ...
      block_info[i].lb[n] = (xprobmin[n] +
                             (block_info[i].ind[n]) * dx1[n] /
                                 pow(2., (double)block_info[i].level - 1.));
#if (DEBUG)
      if (isnan(block_info[i].lb[n])) {
        fprintf(stderr, "NaN %d %lf %d", i, pow(2, block_info[i].level - 1),
                block_info[i].level);
        exit(1);
      }
#endif
