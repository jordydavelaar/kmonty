...
#pragma omp parallel private(i,j,k,Thetae, K2, Ne, B, fac_th,fac_nth, l, lstart, lend,myid,nthreads,Ucon,Bcon)
        {
...
#if (THERMAL)
                                K2 = K2_eval(Thetae);
                                fac_th = (JCST * Ne * B * Thetae * Thetae /
                                          K2) * sfac * geom[i][j].g;
#elif (KAPPA|| POWERLAW)
              ...
              //          fprintf(stderr,"%g %g %g %g\n",Bmag,Thetae,lbth,(lbth - lb_min)/dlb);
                ...
                *dnmax = 0.;
                for (l = 0; l <= N_ESAMP; l++) {
                        ...
                        if(ACCZONE) {
                                dn = perct_thermal * F_eval_th(Thetae, Bmag, exp(l * dlnu + lnu_min)) / (exp(wgt[l]))
...
/* Return the next zone and the number of superphotons that need to be generated in it.  */
	int in2gen;
	double n2gen;
	static int zi = 0;
	static int zj = 0;
