...
double calc_norm(double thetae) {
  // we integrate over beta since this is nicely bound betwoon 0 and 1, indsted
  // of 1 and infinity.
  ...
  gsl_integration_qag(&integrand, 0, log(1. + 100. * thetae), absolute_error,
    ...
    for (gammae = 1. + 0.5 * dgammae; gammae < 1. + MAXGAMMA * thetae;
      ...
      if (isnan(cross)) {
        fprintf(stderr, "%g %g %g %g %g %g\n", w, thetae, mue, gammae,
                dNdgammae(thetae, gammae), boostcross(w, mue, gammae));
      }
    }
