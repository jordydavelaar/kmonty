...

  {
    ...
    for (i = 0; i < N1; i++) {
      for (j = 0; j < N2; j++)
        for (k = 0; k < N3; k++) {
          igrid = i;
          get_fluid_zone(i, j, k, &Ne, &Thetae, &B, &sigma, &beta, Ucon, Bcon,
                         &ACCZONE, &dx_local, &igrid);
          sfac = dx_local * dx_local * dx_local * L_unit * L_unit * L_unit;
...
// thermal
			...
			fac_nth = (EE * EE * EE / (2. * M_PI * ME * CL * CL)) *
                                  Ne * B * sfac * geom[i][j].g;
          #elif(MIXED) 
			K2 = K2_eval(Thetae);
		        fac_th = (JCST * Ne * B * Thetae * Thetae / K2) * sfac * geom[i][j].g;
		        fac_nth = (EE * EE * EE / (2. * M_PI * ME * CL * CL)) * Ne * B *
                	sfac * geom[i][j].g;
          #endif 
		...
		sum[l] += fac_nth * F_eval_nth(Thetae, B, nu[l]);
                ...
                if (ACCZONE) {
              sum[l] +=
                  perct_thermal * fac_th * F_eval_th(Thetae, B, nu[l]) +
		   ...
		   sum[l] += fac_th * F_eval_th(Thetae, B, nu[l]);
            #endif
		if(isnan(sum[l]))
			fprintf(stderr,"fac_th %e K2 %e Thetae %e B %e Ne %e nu[l] %e F_eval %e\n",fac_th,K2,Thetae,B,Ne,nu[l],F_eval_th(Thetae, B, nu[l]));
          }
          //																																				fprintf(stderr,"%e
          //%e %e %e %e %e %e\n",sum[0],fac_th,geom[i][j].g,K2,Thetae,Ne,B);
        }
    }
  }
...
static void init_zone(int i, int j, int k, double *nz, double *dnmax) {
  ...
  if (Ne == 0. || Thetae < THETAE_MIN ) {
    *nz = 0.;
    *dnmax = 0.;
    //										fprintf(stderr,"plasma variables too small or too
    //large %e %e\n",Thetae,Ne);
    return;
  }
#if(POWERLAW)
  ...
  if (l < 0) {
    *dnmax = 0.;
    *nz = 0.;
    fprintf(stderr,"l smaller than zero l %d lbth %e dlb %e Bmag %e Thetae %e\n",l,lbth,dlb,Bmag,Thetae);
    return;
  } else
 if (l >= NINT || MIXED || POWERLAW ) {
    		//	fprintf(stderr,	"warning: outside of nint table range %g...change in harm_utils.c\n", Bmag * Thetae * Thetae);
    ninterp = 0.;
    *dnmax = 0.;
    for (l = 0; l <= N_ESAMP; l++) {
      #if(KAPPA||POWERLAW)
        dn = F_eval_nth(Thetae, Bmag, exp(l * dlnu + lnu_min)) / exp(wgt[l]);
      #elif(THERMAL) 
	dn =
          F_eval_th(Thetae, Bmag, exp(l * dlnu + lnu_min)) / exp(wgt[l]);
      #elif(MIXED) 
	K2 = K2_eval(Thetae);
      if (K2 == 0.){
        *nz = 0.;
      ...
      if(ACCZONE){
        dn = perct_thermal * F_eval_th(Thetae, Bmag, exp(l * dlnu + lnu_min)) /
                 (exp(wgt[l] + 1e-100)) +
    ...
    //								fprintf(stderr,"ninter is %e fac_th is %e dx_loca is
    //%e",ninterp,fac_th,dx_local);
  ...
  *nz = geom[i][j].g * fac_mixed * ninterp;
    ...
    fprintf(stderr, "Something very wrong in zone %d %d: \nB=%g  Thetae=%g  "
                    "K2=%g  ninterp=%g\n\n",
  ...
  //			fprintf(stderr,"results are nz %e dnmax %e bmag %e thetae %e Ne
  //%e\n",*nz,*dnmax, Bmag,Thetae,Ne);
  return;
}
...
int get_zone(int *i, int *j, int *k, double *dnmax) {
   ...
   * generated in it.                                */
  //				fprintf(stderr,"im in get zone now\n");
...
void sample_zone_photon(int i, int j, int k, double dnmax,
  ...
  } while (monty_rand() > (F_eval(Thetae, Bmag, nu, ACCZONE) / weight) / dnmax);
  ...
  do {
    cth = 2. * monty_rand() - 1.;
    th = acos(cth);
  } while (monty_rand() > jnu_synch(nu, Ne, Thetae, Bmag, th, ACCZONE) / jmax);
  ...
  E = nu * HPL / (ME * CL * CL);
  K_tetrad[0] = E;
/*  K_tetrad[1] = E * cphi*cth;
  K_tetrad[2] = E *  sth;
  K_tetrad[3] = E * sphi * cth;
*/
        K_tetrad[0] = E;
        K_tetrad[1] = E * cth;
...
/*
  ...
  K_tetrad[3] = E * cth;
*/
  double R2 = ph->X[1] * ph->X[1] + ph->X[2] * ph->X[2] + ph->X[3] * ph->X[3];
  double a2 = a * a;
  double r2 = (R2 - a2 + sqrt((R2 - a2) * (R2 - a2) + 4. * a2 * ph->X[3]*ph->X[3])) *
              0.5;
  double r = sqrt(r2);
 ...
 // if(ph->E <0){
//	fprintf(stderr,"E wrong!!! E %e nu %e Te %e B %e ne %e r %e\n",ph->E,nu,Thetae,Bmag,Ne,(r));
//  }
...
void init_storage(void) {
  ...
  int j, k;
  shared_spect =
      (struct of_spectrum ***)malloc(N_THBINS * sizeof(struct of_spectrum **));
  for (i = 0; i < N_THBINS; i++) {
    shared_spect[i] =
        (struct of_spectrum **)malloc(N_PHIBINS * sizeof(struct of_spectrum *));
