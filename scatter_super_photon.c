...
void scatter_super_photon(struct of_photon *ph, struct of_photon *php,
                          ...
                          int ACCZONE) {
  double P[NDIM], Econ[NDIM][NDIM], Ecov[NDIM][NDIM], K_tetrad[NDIM],
      K_tetrad_p[NDIM], Bhatcon[NDIM], tmpK[NDIM];
  int k;
  ...
  /* quality control */
  if (ph->K[0] > 1.e5 || ph->K[0] < 0. || isnan(ph->K[1]) || isnan(ph->K[0]) ||
      isnan(ph->K[3])) {
    fprintf(stderr, "normalization problem, killing superphoton: %g \n",
            ph->K[0]);
    ph->K[0] = fabs(ph->K[0]);
  ...
  /* make local tetrad */
  make_tetrad(ph->X, Ucon, Bhatcon, Gcov, Econ, Ecov);
  ...
  if (K_tetrad[0] > 1.e5 || K_tetrad[0] < 0. || isnan(K_tetrad[1])) {
    fprintf(stderr, "conversion to tetrad frame problem: %g %g\n", ph->K[0],
            K_tetrad[0]);
    /*        fprintf(stderr,"%g %g %g\n",ph->K[1], ph->K[2], ph->K[3]);
       fprintf(stderr,"%g %g %g\n",K_tetrad[1], K_tetrad[2], K_tetrad[3]);
       fprintf(stderr,"%g %g %g %g\n",Ucon[0], Ucon[1], Ucon[2], Ucon[3]);
       fprintf(stderr,"%g %g %g %g\n",Bhatcon[0], Bhatcon[1], Bhatcon[2],
       Bhatcon[3]);
       fprintf(stderr,"%g %g %g %g\n", Gcov[0][0], Gcov[0][1], Gcov[0][2],
       Gcov[0][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Gcov[1][0], Gcov[1][1], Gcov[1][2],
       Gcov[1][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Gcov[2][0], Gcov[2][1], Gcov[2][2],
       Gcov[2][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Gcov[3][0], Gcov[3][1], Gcov[3][2],
       Gcov[3][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Ecov[0][0], Ecov[0][1], Ecov[0][2],
       Ecov[0][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Ecov[1][0], Ecov[1][1], Ecov[1][2],
       Ecov[1][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Ecov[2][0], Ecov[2][1], Ecov[2][2],
       Ecov[2][3]) ;
       fprintf(stderr,"%g %g %g %g\n", Ecov[3][0], Ecov[3][1], Ecov[3][2],
       Ecov[3][3]) ;
       fprintf(stderr,"X1,X2: %g %g\n",ph->X[1],ph->X[2]) ;*/
    ph->w = 0.;
    return;
  ...
  sample_electron_distr_p(K_tetrad, P, Thetae, ACCZONE);
  ...
  /* quality control */
  if (isnan(php->K[1])) {
    fprintf(stderr, "problem with conversion to coordinate frame\n");
    fprintf(stderr, "%g %g %g %g\n", Econ[0][0], Econ[0][1], Econ[0][2],
            Econ[0][3]);
    fprintf(stderr, "%g %g %g %g\n", Econ[1][0], Econ[1][1], Econ[1][2],
            Econ[1][3]);
    fprintf(stderr, "%g %g %g %g\n", Econ[2][0], Econ[2][1], Econ[2][2],
            Econ[2][3]);
    fprintf(stderr, "%g %g %g %g\n", Econ[3][0], Econ[3][1], Econ[3][2],
            Econ[3][3]);
    fprintf(stderr, "%g %g %g %g\n", K_tetrad_p[0], K_tetrad_p[1],
            K_tetrad_p[2], K_tetrad_p[3]);
    php->w = 0;
    return;
  }
  ...
  /* bookkeeping */
  K_tetrad_p[0] *= -1.;
  // was Ecov? probably found a bug in the original version
  tetrad_to_coordinate(Econ, K_tetrad_p, tmpK);
