...
double dfdgam(double ge, void *params) {
    ...
    return 2. + ge * (ge / (ge * ge - 1) - 1. / gamma_max -
                      (kap + 1) / kap / w / (1. + (ge - 1.) / kap / w));
#else
...
double fdist(double ge, double Thetae) {
    ...
    double kap = kappa_synch;
    double w = (kap - 3.) / kap * Thetae;
    return ge * ge * sqrt(ge * ge - 1.) *
           pow(1. + (ge - 1.) / (kap * w), -kap - 1.) * exp(-ge / gamma_max);
#else
...
void sample_beta_distr_num(double Thetae, double *gamma_e, double *beta_e) {
    ...
    double kap = kappa_synch;
    double w = (kap - 3.) / kap * Thetae;
    // Relativistic kappa distribution does not like very small Thetae. Ugly
    // kludge.
    if (w < 0.01) {
        *gamma_e = 1.000001;
