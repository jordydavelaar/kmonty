...
void init_sphere_data(char *fname) {
    ...
    for (int i = 0; i < N1; i++) {
        for (int j = 0; j < N2; j++) {
            for (int k = 0; k < N3; k++) {
                ...
                //            fprintf(stderr,"rho %e Ne %e Ne_unit %e U
                //            %e\n",p[KRHO][i][j][k],Ne,Ne_unit,p[UU][i][j][k]);
                //          fprintf(stderr,"B1 %e B2
    ...
    /*      V = dMact = Ladv = 0.;
          ...
          ZLOOP {
                  ...
                  //                                    printf("%d %d %e %e %e
       %e\n",i,j,X[1],RSPHERE,p[UU][i][j],Thetae_unit); V += dV*geom[i][j].g;
                  bias_norm +=
       dV*geom[i][j].g*pow(p[UU][i][j]/p[KRHO][i][j]*Thetae_unit,2.);

                  if (i <= 20) {M_PI/2.
                                double Ne, Thetae, Bmag, Ucon[NDIM], Ucov[NDIM],
       Bcon[NDIM]; get_fluid_zone(i, j, &Ne, &Thetae, &Bmag, Ucon,
       Bcon,&ACCZONE); lower(Ucon, geom[i][j].gcov, Ucov); dMact +=
       geom[i][j].g*dx[2]*dx[3]*p[KRHO][i][j]*Ucon[1]; Ladv +=
       geom[i][j].g*dx[2]*dx[3]*p[UU][i][j]*Ucon[1]*Ucov[0]; }
          }
